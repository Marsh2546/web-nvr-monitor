...
export function Dashboard() {
  ...
  const loadData = () => {
    ...
    const newStats = {
      ...
      pending: allRequests.filter(r => r.status === 'รอดำเนินการ').length,
      inProgress: allRequests.filter(r => r.status === 'กำลังซ่อม').length,
      completed: allRequests.filter(r => r.status === 'เสร็จสิ้น').length,
      cancelled: allRequests.filter(r => r.status === 'ยกเลิก').length,
    };
    setStats(newStats);
  };
