...
import { AnimatedNumber, cn, calculateEffectiveStatus, getIssueStatus, hasCriticalIssues, hasAttentionIssues, hasIssues, NVRWithIssues } from "@/app/utils";
import {
  Server,
  CheckCircle,
  AlertTriangle,
...
export function NVRDashboard({ nvrList, onPageChange }: NVRDashboardProps) {
  ...
  const chartData = useMemo(() => {
    ...
    const districtData = nvrWithIssues.reduce(
      (acc, nvr) => {
        const existing = acc.find((item: any) => item.district === nvr.district);
        if (existing) {
          existing.total += 1;
